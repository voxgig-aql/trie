...
# across a series of key counts. Reports best-of-1 wall time. Requires `aql`
...
timerun() { local mode="$1" s e; s=$(date +%s.%N); aql "$mode" "$BENCH" >/dev/null 2>&1; e=$(date +%s.%N); awk "BEGIN{printf \"%.2f\", $e-$s}"; }
