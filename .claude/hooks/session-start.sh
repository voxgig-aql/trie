...
have_ver="$("$BORU" -version 2>/dev/null || boru -version 2>/dev/null || true)"
if [ -n "$have_ver" ] && { [ "${have_ver#boru }" = "$BORU_REF" ] || [ -z "$BORU_REF" ]; }; then
  log "boru already present ($have_ver); skipping build."
else
  if [ -z "$BORU_REF" ]; then
...
# Fast confidence check: run the smoke demo if boru is usable. Never fail the
# session on a check error.
if [ -x "$BORU" ] && [ -f "$CLAUDE_PROJECT_DIR/test/trie_smoke_test.aql" ]; then
  if ( cd "$CLAUDE_PROJECT_DIR" && "$BORU" test/trie_smoke_test.aql >/dev/null 2>&1 ); then
